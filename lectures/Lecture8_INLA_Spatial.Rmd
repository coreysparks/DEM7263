...
files<-list.files("~/OneDrive - University of Texas at San Antonio//classes/dem7263/class21_7263/data/nhgis0022_csv/nhgis0022_csv/", pattern = "*.csv", full.names = T)
# vital<-lapply(files, 
#               read.csv, 
#               header=T)
# 
...
```{r}
ggplot(data = final.dat)+geom_histogram(aes(x =log(rate) ,
                                            y=0.5*..density..))+
  #facet_wrap(~year)+
  ggtitle(label = "Distribution of Infant Mortality Rate by Year",
          subtitle = "US Counties, 2000")

ggplot(data = final.dat)+geom_histogram(aes(x =log(rate/E_d) ,
                                            y=0.5*..density..))+
  #facet_wrap(~year)+
...
final.dat%>%
  ...
  mutate(qrr=cut(I(rate/E_d), 
                 breaks = quantile(I(rate/E_d),
...
```{r}
#Model specification:
f1<-rate~scale(pblack)+scale(phisp)+scale(ppov)
...
```{r}
f2<-rate~scale(pblack)+scale(phisp)+scale(ppov)+ #fixed effects
...
$$\text{Deaths_ij} = \text{log(E_d)} + X' \beta + u_j + v_j $$
...
final.dat%>%
 # filter(year%in%c(2000))%>%
  ...
  ggtitle(label=expression(paste("Exceedence Probability Relative Risk ","Pr( ",theta," >1.25"," )  - 2000") ))+
  coord_sf(crs = 2163)
...
f4<-rate~scale(pblack)+scale(phisp)+scale(ppov)+
  f(struct, model = "bym",scale.model = T, constr = T, graph = H,
    hyper = prec.prior)
